...
│   ├── agent/                # multi-turn loop, tool execution, compaction, capability seams (PromptBuilder, Compactor, ToolProvider, SessionStore), extensions; events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent
│   ├── harness/              # harness layer: system prompt building, skill loading, project context loading
...
| `internal/ai/`      | Implemented | Provider abstraction, stream events, message + tool types (including `ModelChange`, `ThinkingLevelChange` session entries), retry, multi-provider, API key auth (Ollama Cloud, OpenAI, Anthropic), HTTP timeout (OMEGA_HTTP_TIMEOUT)                                                                                                                                                        |
| `internal/agent/`   | Implemented | Multi-turn loop, tool execution (per-path file locks), compaction (threshold + overflow auto-retry, extension customization hooks), capability seams (`PromptBuilder`, `Compactor`, `ToolProvider`, `SessionStore`), **extensions** (customization + seam replacement); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent |
| `internal/harness/` | Implemented | Harness layer: system prompt building (`BuildSystemPrompt`), skill loading (`LoadSkills`), project context loading (`LoadProjectContext`, `ProjectRoot`)                                                                                                                                                                                                                                    |
| `internal/gateway/` | Implemented | HTTP server, SSE streaming, session store, config, session tree, cross-session insights (`ComputeInsights`); SSE events: agent_start, turn_start, response_chunk, thinking_chunk, tool_call, stream_end, assistant_message, tool_result, turn_end, agent_end                                                                                                                                |
...
| `extensions/`       | Reference   | Extension authoring guide (README.md) and example web extension                                                                                                                                                                                                                                                                                                                             |
