...
| `agent/`          | Implemented | Multi-turn loop (AgentLoop interface + DefaultAgentLoop), tool execution, compaction, capability seams (Compactor, ToolProvider, AgentLoop, StoreProvider, SkillsProvider), extensions (StdioManager, ExtensionManager, ProxyStore, ProxySkills); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, ToolResultEvent, SessionEvent; shared data types (Session, SessionNode, Insights, SearchResult)        |
