...
├── agent/                     # multi-turn loop (AgentLoop interface + DefaultAgentLoop), tool execution, compaction, capability seams (PromptBuilder, Compactor, ToolProvider, SessionStore, AgentLoop), extensions; events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent
...
| `agent/`          | Implemented | Multi-turn loop (`AgentLoop` interface + `DefaultAgentLoop`), tool execution (per-path file locks), compaction (threshold + overflow auto-retry, extension customization hooks), capability seams (`PromptBuilder`, `Compactor`, `ToolProvider`, `SessionStore`, `AgentLoop`), **extensions** (customization + seam replacement); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent |
...
| `bin/extensions/` | Reference   | Extension authoring guide (README.md), core tools extension, web search extension, MCP bridge, and skill templates                                                                                                                                                                                                                                                                                                                                    |     |
