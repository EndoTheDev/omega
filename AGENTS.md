...
├── agent/                     # multi-turn loop (AgentLoop interface + DefaultAgentLoop), tool execution, compaction, capability seams (Compactor, ToolProvider, AgentLoop), extensions; events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent
...
| `bin/extensions/` | Reference   | Extension authoring guide (README.md), core-prompt extension (system prompt builder), core-provider extension (LLM provider streaming: Ollama, OpenAI, Anthropic), core-store extension (SQLite session store, FTS5 search, sessions.search tool), core tools extension, web search extension, MCP bridge, and skill templates                                                                                       |     |
