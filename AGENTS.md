...
| `internal/agent/`   | Implemented | Multi-turn loop, tool execution (per-path file locks), compaction (threshold + overflow auto-retry), project context, system prompt, skills, **extensions**; events: AgentStart, TurnStart, TurnEnd, AgentEnd (carries assistant message), StreamEvent, AssistantMessageEvent, ToolResultEvent                                                                                    |
