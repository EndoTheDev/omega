...
| `internal/agent/`   | Implemented | Multi-turn loop, tool execution (per-path file locks), compaction (threshold + overflow auto-retry, extension customization hooks), project context, system prompt (extension guidelines), skills, **extensions** (customization hooks: prompt guidelines, compaction, branch summary, session lifecycle); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent |
