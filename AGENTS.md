...
└── cmd/
    └── omega/                # single binary: serve, run, health, chat (no arg → TUI, `<path>` → chdir + TUI); project trust (--approve/--no-approve, trust.yaml); export, update; `@file` image input; /new --ephemeral, /sessions (table, delete, resume by #/label/id), /tree (table), /copy, /export, /thinking, /tools, /extensions, /skills, /models, /model <#|name>
```
...
| `cmd/omega/`        | Implemented | Single binary: serve, run, health, chat (no arg → TUI, `<path>` → chdir + TUI); project trust (--approve/--no-approve, trust.yaml); export, update; `@file` image input; /new --ephemeral, /sessions (table, delete, resume by #/label/id), /tree (table), /copy, /export, /thinking, /tools, /extensions, /skills, /theme, /models, /model (number or name) |
