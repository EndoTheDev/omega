...
├── agent/                     # multi-turn loop (AgentLoop interface + DefaultAgentLoop), tool execution, compaction, capability seams (Compactor, ToolProvider, SessionStore, AgentLoop), extensions; events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent
├── harness/                   # harness layer: skill loading, project context loading
...
│   │   ├── core-tools/        # core tools extension (shell, files, skills)
...
| `ai/`             | Implemented | Provider abstraction, stream events, message + tool types (including `ModelChange`, `ThinkingLevelChange` session entries), retry, multi-provider, API key auth (Ollama Cloud, OpenAI, Anthropic), HTTP timeout (OMEGA_HTTP_TIMEOUT)                                                                                                                                                                                                 |
| `agent/`          | Implemented | Multi-turn loop (`AgentLoop` interface + `DefaultAgentLoop`), tool execution (per-path file locks), compaction (threshold + overflow auto-retry, extension customization hooks), capability seams (`Compactor`, `ToolProvider`, `SessionStore`, `AgentLoop`), **extensions** (customization + seam replacement); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent |
| `harness/`        | Implemented | Harness layer: skill loading (`LoadSkills`), project context loading (`LoadProjectContext`, `ProjectRoot`)                                                                                                                                                                                                                                                                                                                           |
...
| `bin/extensions/` | Reference   | Extension authoring guide (README.md), core-prompt extension (system prompt builder), core tools extension, web search extension, MCP bridge, and skill templates                                                                                                                                                                                                                                                                    |     |
