...
func newChatModel(modelName, host string, compaction *agent.CompactionConfig, store *gateway.Store) model {
	ta := textarea.New()
...
func runChat(modelName, host string, compaction *agent.CompactionConfig, store *gateway.Store) error {
	m := newChatModel(modelName, host, compaction, store)
